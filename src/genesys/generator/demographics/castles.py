...
def ruins(population=6600000, age=300):
    """
    Ruins, first of all, depend on the age of the region. The following formula is only a guide. The frequency of ruins
    in Europe varied greatly depending on military history and remoteness of the area. To determine the approximate
    number of ruined fortifications, divide the kingdom's population by five million. Multiply the result by the square
    root of the kingdom's age. If the kingdom has changed hands a lot, use the total age-the number of years that
    ...
    :return:
    """
    ruins_count = (population / 5000000) * math.sqrt(age)
    res = int(ruins_count)
    if random.randrange(100) / 100 < ruins_count - res:
        res += 1
    return res


def castles(population=6600000):
    """
...
def civilized(castles=133):
    ...
    of Barons and Dukes, but some may be bandit strongholds, or the outposts of Goblin warlords. It is all up to the
    GM.
