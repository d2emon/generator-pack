...
class NameFactory:
    ...
    def get_name_data(self):
        if isinstance(self.namegen, str):
            return self.namegen
        elif isinstance(self.namegen[0], str):
            return random.choice(self.namegen)
        else:
    ...
    def __call__(self, *args, **kwargs):
        name_parts = self.get_name_data().split('|')
        name = self.get_name(name_parts[0])
...
class PersonNameFactory(NameFactory):
...
class MedievalNameFactory(PersonNameFactory):
...
class AncientNameFactory(PersonNameFactory):
...
class FutureNameFactory(PersonNameFactory):
...
class MemoryFactory(NameFactory):
...
class ThoughtFactory(NameFactory):
    pass
...
class MedievalMemoryFactory(MemoryFactory):
...
class MedievalThoughtFactory(ThoughtFactory):
...
class AncientMemoryFactory(MemoryFactory):
...
class AncientThoughtFactory(ThoughtFactory):
...
class FutureMemoryFactory(MemoryFactory):
...
class FutureThoughtFactory(ThoughtFactory):
...
class PaintingFactory(NameFactory):
...
class MonumentFactory(NameFactory):
