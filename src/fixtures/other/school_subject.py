school_subjects = [
    "Advanced Mathematics",
    "Alchemy",
    "Alien Anatomy",
    "Alien Anthropology",
    "Alien Archaeology",
    "Alien Arts",
    "Alien Biochemistry",
    "Alien Bioengineering",
    "Alien Biology",
    "Alien Biosecurity",
    "Alien Biotechnology",
    "Alien Calligraphy",
    "Alien Cellular Biology",
    "Alien Child Psychology",
    "Alien Civilization Studies",
    "Alien Computer Science",
    "Alien Crafting",
    "Alien Criminal Justice",
    "Alien Criminology",
    "Alien Culinary Arts",
    "Alien Dance",
    "Alien Dance History",
    "Alien Dialects",
    "Alien Diplomacy",
    "Alien Disaster Management",
    "Alien Drama",
    "Alien Ecology",
    "Alien Economics",
    "Alien Engineering",
    "Alien Environmental Development",
    "Alien Ethics",
    "Alien Eurythmy",
    "Alien Evolutionary Biology",
    "Alien Family Psychology",
    "Alien Fashion",
    "Alien Finance",
    "Alien Forensic Science",
    "Alien Genealogy",
    "Alien Government Studies",
    "Alien Healthcare Practice",
    "Alien Heritage",
    "Alien History",
    "Alien Instrumental Music",
    "Alien Language",
    "Alien Language Creative Writing",
    "Alien Language Culture",
    "Alien Language History",
    "Alien Language Literature",
    "Alien Language Poetry",
    "Alien Law and Politics",
    "Alien Life Gardening",
    "Alien Life Physics",
    "Alien Life Science",
    "Alien Linguistics",
    "Alien Logic",
    "Alien Martial Arts",
    "Alien Mathematics",
    "Alien Mechanics",
    "Alien Medical Physics",
    "Alien Medicine",
    "Alien Musical Arts",
    "Alien Neuroscience",
    "Alien Nutrition",
    "Alien Organic Chemistry",
    "Alien P.E.",
    "Alien Pathology",
    "Alien Philosophy",
    "Alien Physiology",
    "Alien Political Sciences",
    "Alien Psychology",
    "Alien Religion",
    "Alien Resource Management",
    "Alien Science",
    "Alien Services",
    "Alien Social Sciences",
    "Alien Social Skills",
    "Alien Social Studies",
    "Alien Sociology",
    "Alien Speech",
    "Alien Sports",
    "Alien Sports History",
    "Alien Statistics",
    "Alien Survival Techniques",
    "Alien Tactics and Strategy",
    "Alien Technology Studies",
    "Alien Therapy",
    "Alien Veterinary Science",
    "Animal Care",
    "Apparition",
    "Army Strategy",
    "Astronomy",
    "Audiology",
    "Bioengineering",
    "Business Technology",
    "Charm Casting",
    "Charm Creation",
    "Combat Riding",
    "Counter Spellcasting",
    "Curse Casting",
    "Curse Creation",
    "Dark Arts",
    "Dead Language",
    "Dead Language Culture",
    "Dead Language History",
    "Dead Language Literature",
    "Demonology",
    "Dimensional Manipulation",
    "Disaster Management",
    "Divining",
    "Earth Magic",
    "Earth Science",
    "Elemental Magic",
    "Endurance Training",
    "Enhanced First Aid",
    "Enhanced Therapy",
    "Eurythmic Language",
    "Extinct Language",
    "Extinct Language Culture",
    "Extinct Language History",
    "Extinct Language Literature",
    "First Aid",
    "Flying",
    "Foreign Anatomy",
    "Foreign Anthropology",
    "Foreign Archaeology",
    "Foreign Arts",
    "Foreign Biochemisty",
    "Foreign Bioengineering",
    "Foreign Biology",
    "Foreign Biosecurity",
    "Foreign Biotechnology",
    "Foreign Calligraphy",
    "Foreign Civilization Studies",
    "Foreign Crafting",
    "Foreign Criminal Justice",
    "Foreign Criminology",
    "Foreign Culinary Arts",
    "Foreign Dance",
    "Foreign Dance History",
    "Foreign Dialects",
    "Foreign Diplomacy",
    "Foreign Disaster Management",
    "Foreign Drama",
    "Foreign Ecology",
    "Foreign Economics",
    "Foreign Engineering",
    "Foreign Environmental Development",
    "Foreign Ethics",
    "Foreign Eurythmy",
    "Foreign Evolutionary Biology",
    "Foreign Fashion",
    "Foreign Finance",
    "Foreign First Aid",
    "Foreign Forensic Science",
    "Foreign Government Studies",
    "Foreign Healthcare Practice",
    "Foreign Heritage",
    "Foreign History",
    "Foreign Instrumental Music",
    "Foreign Language Poetry",
    "Foreign Law and Politics",
    "Foreign Life Gardening",
    "Foreign Life Physics",
    "Foreign Life Science",
    "Foreign Linguistics",
    "Foreign Martial Arts",
    "Foreign Mechanics",
    "Foreign Medical Physics",
    "Foreign Medicine",
    "Foreign Musical Arts",
    "Foreign Neuroscience",
    "Foreign Nutrition",
    "Foreign Organic Chemistry",
    "Foreign P.E.",
    "Foreign Pathology",
    "Foreign Philosophy",
    "Foreign Planet Science",
    "Foreign Political Sciences",
    "Foreign Psychology",
    "Foreign Religion",
    "Foreign Resource Management",
    "Foreign Services",
    "Foreign Social Skills",
    "Foreign Social Studies",
    "Foreign Speech",
    "Foreign Sports",
    "Foreign Sports History",
    "Foreign Statistics",
    "Foreign Survival Techniques",
    "Foreign Tactics and Strategy",
    "Foreign Technology Studies",
    "Foreign Therapy",
    "Forestry",
    "Galactic Business",
    "Galactic Diplomacy",
    "Galactic History",
    "Grand Strategy",
    "Handcrafts",
    "Handwriting",
    "Healing Magic",
    "Health and Safety",
    "Herbalism",
    "Heritage",
    "Horse Riding",
    "Intergalactic Astronomy",
    "Intergallactic Relations",
    "Leadership",
    "Life Gardening",
    "Magic Arts",
    "Magic History",
    "Magic Infusion",
    "Magic Law",
    "Magic Music",
    "Magic Rituals",
    "Magic Sports",
    "Magic Survival",
    "Martial Arts",
    "Mathamancy",
    "Mathematics",
    "Mental Projection",
    "Mentalist Magic",
    "Military Competitions",
    "Military History",
    "Military Law",
    "Military Legacy",
    "Military Magic",
    "Military Philosophy",
    "Military Tactics",
    "Monster Hunting",
    "Monster Recognition",
    "Monster Transformations",
    "Mount Care",
    "Necromancy",
    "Nutrition",
    "Nutrition Recognition",
    "Physical Science",
    "Planetary Animal Science",
    "Planetary Biochemistry",
    "Planetary Biology",
    "Planetary Botany",
    "Planetary Business",
    "Planetary Business Economics",
    "Planetary Business Management",
    "Planetary Chemistry",
    "Planetary Ecology",
    "Planetary Environmental Science",
    "Planetary Geography",
    "Planetary History",
    "Planetary Meteorology",
    "Planetary Oceanography",
    "Planetary Physics",
    "Planetary Survival",
    "Planetary Terraforming",
    "Planetary Zoology",
    "Potions",
    "Raid Strategy",
    "Riding",
    "Rituals and Rites",
    "Scavenging",
    "Self Defense, Hand to Hand",
    "Self Defense, Magic",
    "Self Defense, Monsters",
    "Self Defense, Small Weaponry",
    "Self Defense, Weaponry",
    "Siege Mechanics",
    "Small Forces Strategy",
    "Space Survival",
    "Space Travel",
    "Speech",
    "Speechless Spellcasting",
    "Spirituality",
    "Stealth and Camouflage",
    "Stealth and Survival",
    "Supernatural Studies",
    "Survival",
    "Tactics and Strategy",
    "Telepathy",
    "Teleportation",
    "Terraforming",
    "Transformation",
    "Transmutation",
    "Ward Casting",
    "Ward Creation",
    "Wilderness Survival",
]