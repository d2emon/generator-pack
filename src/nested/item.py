...
class Item:
    ...
    def grow(self, *args, **kwargs):
        def get_data(data):
            ...
            if len(value) > 1:
                amount = get_amount(value[1])

            if len(value) > 1:
                probability = get_probability(data[1])
            else:
                probability = get_probability("1")
            ...
            return value[0], probability, amount
        ...
        for data in self.type.contains:
            # if not instanceof(data, str):
            #     data = Choose(data)
            value, probability, amount = get_data(data)
            print(value, probability, amount)

            if Thing.get_thing(value) is None:
                continue
            ...
            for i in range(amount):
                new_item = Item(Thing.get_thing(value).name)
